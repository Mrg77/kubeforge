...
- **Topology** — two lenses over the cluster:
  ...
  - **Heatmap** — one cell per pod, colored by health (and amber when it's restarting a lot), for fleet status at scale (500+ pods). Cells sort problems to the top-left; hover for the pod's identity, node and restarts; click to open its events.
